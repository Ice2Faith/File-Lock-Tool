...
void DirLock(const char dirroute[],const char dirname[],const char preworkdir[],const char password[],const char usersel)
{
    char mind[2048]= {'\0'};
    char workdir[2048]= {0};
    sprintf(workdir,"%s\\%s",preworkdir,dirname);
    sprintf(mind,"md \"%s\"\0",workdir);
    system(mind);

    struct _finddata_t predir;
    long handle;
    ...
    while(_findnext(handle,&predir)==0);
    _findclose(handle);
}
void PathEnvinoment(char UserRoute[])
{
    ...
    FILE * Path=fopen("C:\\FileLockToolPro\\UserPath\\Path.xml","r");
    if(!Path)
    {
        fclose(Path);
