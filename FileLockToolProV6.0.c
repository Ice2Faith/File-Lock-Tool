...
#define FALSE 0
#define WORD char
#define APPNAME "FileLockToolProV6.0.exe"
...
{
    FaceInt("f5","FileLockToolProV6.0 Dev: Ice2Faith Ver: 6.0",85,25);
    if(num==1)
    {
        UserGuid();
    }
...
{
    //file define format:
    //Ice2Faith_FileLocker_int_key_body
    //design :
    //get length of "Ice2Faith_FileLocker_" char to save
...
void UserGuid()
{
    ...
    printf("\n\tVer: 6.0\n\n");
    printf("\n\tDev: Ice2Faith\n\n");
    printf("\n\n\tClick any key continue jump to help:\n>/ ");
    getch();
    for(int i=0; i<5; i++)
    {
        system("cls");
        printf("\n\t使用引导及说明:\n\n\t请将待加密/解密文件拖动到程序图标上启动\t( ^_^)／\n\n");
...
void ControlCenter(int num,char * string[])
{
    ...
    if(del=='*')
    {
        ...
        fputs("@echo off\n",Bat);
        fputs("echo Builder: FileLockToolProV6.0\n",Bat);
        fputs("echo Ver: 6.0\n",Bat);
    ...
    fputs(">>>>>>>>>>>>\tIce2Faith FileLockToolProV6.0 informations\n\n",Password);
    fprintf(Password,"Ver: \t\t6.0\n\n",Password);
